...
$id = 'blockhaus-project-leads-' . $block['id'];
if( !empty($block['anchor']) ) {
    $id = $block['anchor'];
}

$profiles = get_field('members_people', $post_id);
  ...
  <div class="flex flex-col p-6 mb-6 w-full h-full border rounded-md shadow-md gap-6">
    ...
    if($further_info):?>
      ...
      <a class="flex gap-2 no-underline w-fit items-center bg-contrast text-white text-sm px-3 py-1 rounded-full hover:ring-2 focus:ring-2 ring-offset-2 ring-transparent hover:ring-contrast focus:ring-contrast" href="<?php echo $further_info; ?>">Further information</a>
